...
# Simple memory cache
_cache = {
    "stats": {"data": None, "expires_at": datetime.min},
    "top_posts": {"data": None, "expires_at": datetime.min},
    "active_users": {"data": None, "expires_at": datetime.min},
}
...
def get_cached_or_compute(key, compute_func):
    now = datetime.utcnow()
    with _cache_lock:
        if _cache[key]["data"] is not None and now < _cache[key]["expires_at"]:
    ...
    with _cache_lock:
        ...
        _cache[key]["expires_at"] = datetime.utcnow() + timedelta(minutes=CACHE_TTL_MINUTES)
...
def get_top_posts(db: Session = Depends(get_db)):
    ...
    def compute_top_posts():
        ...
        for r in results:
            ...
            posts.append({
                ...
                "confession_number": r.confession_number,
    ...
    return get_cached_or_compute("top_posts", compute_top_posts)
...
def get_active_users(db: Session = Depends(get_db)):
    ...
    def compute_active_users():
        results = (
            ...
            .filter(models.User.is_banned == False, models.Post.status == "approved")
    ...
    return get_cached_or_compute("active_users", compute_active_users)
